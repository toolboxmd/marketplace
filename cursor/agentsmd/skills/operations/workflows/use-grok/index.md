...
metadata:
  owner: toolboxmd
  origin: toolboxmd/use-grok
  source-revision: a8ae6ab3c862de836ca576276a221610e3fe274c
...
Select this procedure through Operations when the user explicitly asks to consult Grok. Put the user's question in the brief. Do not run unless the user asked to consult Grok. The separately installed upstream plugin may still expose its own `/use-grok` or `$use-grok` entry; AgentsMD bundles this procedure without a separate discovery entry.
...
Goal [research](../research/index.md). Tell Grok to search the repo and the web, cite paths and URLs, and not edit files. It will use `web_search`, `web_fetch`, `grep`, and `explore` subagents on its own.
