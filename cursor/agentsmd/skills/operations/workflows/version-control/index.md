...
2. Internal component checkpoint: use supported WIP policy under
   [orchestration](../../references/orchestration.md); defer versioning
...
executable `tools/versionctl/bin/versionctl` in the repository. Otherwise resolve the canonical AgentsMD installation from the native global
instruction link and use its executable `bin/versionctl`. A full plugin package
contains it at `../../../../bin/versionctl` relative to this procedure directory.
A Model Router kit may copy only the procedures, so do not infer a package root
from that copy. Stop if no supported executable exists; manual version or mirror
edits are invalid.
...
9. On the clean commit, run `$VERSIONCTL release-check`. If scoped proof is
   declared, use [the scoped-proof contract](../../references/scoped-proof.md) to
